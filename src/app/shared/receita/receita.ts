...
export class Receita {
  ...
  private listaIngredientes(): string[] {
    const ing = this.data().ingredientes_principais;
    const lista: string[] = Array.isArray(ing) ? ing : String(ing ?? '').split(',');
    return lista.map((item: string) => item.trim()).filter(Boolean);
  }

  protected ingredientesTexto(): string {
    return this.listaIngredientes().join(', ');
  }

  protected ingredientesPreview(): string[] {
    return this.listaIngredientes().slice(0, 3);
  }
